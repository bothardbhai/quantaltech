<?php
/**
 * Admin — Hire Submissions
 *
 * Lists leads captured by the Hire Form (pages/hire-submit.php →
 * `hire_submissions`). Modeled directly on admin/newsletter.php — same
 * table-exists guard, same PRG action pattern, same hard-delete convention
 * (no soft-delete exists anywhere in this project).
 */
...
// Check table exists
try {
    $pdo->query('SELECT 1 FROM hire_submissions LIMIT 1');
    $table_exists = true;
...
if ($action === 'update_status' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($id > 0) {
        $pdo->prepare(
            'UPDATE hire_submissions SET status = :st, updated_at = NOW() WHERE id = :id'
        )->execute([':st' => $status, ':id' => $id]);
        flash('success', 'Status updated.');
    }
    header('Location: ' . ADMIN_URL . '/hire-submissions.php'); exit;
}
...
if ($action === 'delete' && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $id = (int) ($_POST['id'] ?? 0);
    if ($id > 0) {
        $pdo->prepare('DELETE FROM hire_submissions WHERE id = :id')->execute([':id' => $id]);
...
if ($action === 'view' && $table_exists) {
    $id = (int) ($_GET['id'] ?? 0);
    $stmt = $pdo->prepare('SELECT * FROM hire_submissions WHERE id = :id');
    $stmt->execute([':id' => $id]);
    $view_row = $stmt->fetch();
    if (!$view_row) {
...
if ($table_exists) {
    $stmt = $pdo->prepare(
        'SELECT id, name, email, phone, company, country, page_url, status, created_at
         FROM hire_submissions' . $where . '
         ORDER BY created_at DESC LIMIT 500'
    );
    $stmt->execute($params);
    $rows = $stmt->fetchAll();

    foreach ($pdo->query('SELECT status, COUNT(*) AS cnt FROM hire_submissions GROUP BY status')->fetchAll() as $r) {
        $stats[$r['status']] = (int) $r['cnt'];
        $stats['total'] += (int) $r['cnt'];
    }
}

$admin_page_title = $action === 'view' ? 'Hire Submission' : 'Hire Submissions';
$admin_active     = 'hire-submissions';
...
<?php if ($action === 'view' && $view_row): ?>
    ...
    <div class="admin-page-header">
        <div>
            ...
            <div class="subtitle"><?= e(substr($view_row['created_at'], 0, 16)) ?></div>
        </div>
        <a href="<?= ADMIN_URL ?>/hire-submissions.php" class="admin-btn admin-btn--ghost">Back to list</a>
    </div>

    <div class="admin-card"><div class="admin-card__body">
        <table class="admin-table">
            <tbody>
                <tr><th style="width:160px;">Name</th><td><?= e($view_row['name']) ?></td></tr>
                ...
                <tr><th>Country</th><td><?= e($view_row['country']) ?></td></tr>
                <tr><th>Hiring Model</th><td><?= e($view_row['hiring_model']) ?></td></tr>
                <tr><th>Project Details</th><td style="white-space:pre-wrap;"><?= e($view_row['project_details']) ?></td></tr>
...
<?php else: ?>
    ...
    <?php if (!$table_exists): ?>
        <div class="flash flash--error">
            Table <code>hire_submissions</code> does not exist.
                ...
                <tbody>
                    <?php if (empty($rows)): ?>
                        <tr><td colspan="9" class="empty-state">
                            ...
                            <p>Hire enquiry submissions will appear here.</p>
                        </td></tr>
                    <?php else: ?>
                        ...
                        <tr>
                            ...
                            <td class="text-muted"><?= e($r['country']) ?></td>
